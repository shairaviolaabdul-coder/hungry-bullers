-- Delivery is free — the ₱120 delivery fee was never a real business rule.
-- Redefine submit_order() with the fee constant at 0. Fulfillment method is
-- still recorded (affects logistics), it just no longer changes price.
...
declare
  ...
  v_delivery_fee_amount constant numeric(10, 2) := 0.00;
...
begin
  ...
  v_delivery_fee := v_delivery_fee_amount; -- always 0.00; fulfillment method no longer affects price
